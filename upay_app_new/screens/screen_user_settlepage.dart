...
class UserSettlePage extends StatelessWidget {
  User user;
  Event event;

  UserSettlePage({Key key, @required this.event, this.user}) : super(key: key);
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: UserSettle(event: event),
    );
  }
}

class UserSettle extends StatefulWidget {
  Event event;

  UserSettle({Key key, this.event}) : super(key: key);

  @override
  UserSettleState createState() {
    return UserSettleState();
  }
}

class UserSettleState extends State<UserSettle> {
  Event event;
        ...
        if (pos[i].balance != 0 && neg[j].balance != 0) {
          if ((pos[i].balance + neg[j].balance) == 0) {
            ...
            swishPayments.add(SwishPayment(
                ...
                message: "Split event: ${widget.event.title}",
                payer: neg[j].name,
                payee: pos[i].name));
  ...
  @override
  Widget build(BuildContext context) {
    sortUserIntoLists(widget.event.guestUsers);
...
//                            leading:,
                ...
                for (int i = 0; i < debtSettled.length; i++) {
                  if (debtSettled[i] == true) {
                    ...
                    Payee payee = new Payee(
                        ...
                        settled: false);
                    List<Payee> myList = List.filled(1, payee);
                    Bill bill = new Bill(
                        title:
                        ...
                        settled: false,
                        payeeList: myList);
                    widget.event.billList.add(bill);
                    for (int i = 0;
                        i < widget.event.guestUsers.length;
                        i++) {
                      if (widget.event.guestUsers[i].name ==
                          bill.payer) {
                        widget.event.guestUsers[i].amountToGet +=
                            bill.value;
                      }
                      for (int j = 0; j < bill.payeeList.length; j++) {
                        if (widget.event.guestUsers[i].name ==
                            bill.payeeList[j].name) {
                          widget.event.guestUsers[i].amountToPay +=
                              bill.payeeList[j].debt;
                        }
                      }
  ...
  final String EVENTS_DB_COLLECTION_NAME = "Events";

  void _updateDb() async {
    _db
        .collection(EVENTS_DB_COLLECTION_NAME)
